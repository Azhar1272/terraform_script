...
limit, rate = 1, 1
url = "https://4918734.suitetalk.api.netsuite.com/services/rest/record/v1/subsidiary"
...
prevday = date.today() + timedelta(days=-5)
...
async def main(urls, rate, limit):
    ...
    retry_urls = await asyncio.gather(*tasks)
    return retry_urls
...
if __name__ == "__main__":
    ...
    if len(urls) > 0:
        ...
        retry_urls = asyncio.run(main(urls, rate=rate, limit=limit))
        if(len(retry_urls)>0):
            retry_urls = asyncio.run(main(retry_urls, rate=rate, limit=limit))
    else:
