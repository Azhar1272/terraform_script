...
'''
def get_response(account_id, consumer_key, consumer_secret, token, token_secret, url):
    nonce = generateNonce()
...
def put_kinesis_firehose(data):
    ...
    stream_name =  'netsuite-data-ingestion-customer'
...
def return_url_list():
    url_list = []
    offset = 0
    total_records = get_response(account_id, consumer_key, consumer_secret, token, token_secret, url)['totalResults']
    print(total_records)
    while (offset < total_records):
        response = json.dumps(get_response(account_id, consumer_key, consumer_secret, token, token_secret, url))
...
async def main():
    ...
    if len(urlx) > 0:
        ...
        print("No records in get api call")
